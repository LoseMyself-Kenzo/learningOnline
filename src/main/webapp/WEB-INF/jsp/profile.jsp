            ...
            <div class="content" style="margin-top: 10px">
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active">
                        <div class="row">
                            <form method="POST" action="${base.contextPath}/changeProfile">
                                <div class="col-md-12 empty-tips" style="margin-top: 20px;margin-left: 80px">
                                    <c:if test="${!empty username}">
                                        昵称:<input type="text" id="userName" name="userName" value="${username}"><br/>
                                    </c:if>
                                    <c:if test="${!empty address}">
                                        地址:<input type="text" id="address" name="address" style="margin-top: 20px" value="${address}"><br/>
                                    </c:if>
                                    <c:if test="${is_work == 0}">
                                        我的状态:<input type="radio" name="isWork" value="0" style="margin-top: 20px" checked>在上学<input type="radio" name="isWork" value="1" style="margin-top: 20px">在工作<br/>
                                    </c:if>
                                    <c:if test="${is_work == 1}">
                                        我的状态:<input type="radio" name="isWork" value="0" style="margin-top: 20px">在上学<input type="radio" name="isWork" value="1" style="margin-top: 20px" checked>在工作<br/>
                                    </c:if>
                                    <c:if test="${!empty school}">
                                        我的学校:<input type="text"id="school" name="school"style="margin-top: 20px" value="${school}"><br/>
                                    </c:if>
                                    <c:if test="${!empty user_id}">
                                        <input type="hidden" name="userId" value="${user_id}"/>
                                    </c:if>
                                </div>
                                <button class="btn btn-primary" type="submit" style="margin-left:75px;margin-top:20px;width: 300px">Save</button>
                            </form>
                        </div>
                    </div>
                </div>
